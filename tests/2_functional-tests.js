    ...
    test("1 stock", function(done) {
      chai
        ...
        .end(function(err, res) {
          assert.equal(res.status, 200, "expected status to equal 200");
          assert.isObject(
            res.body.stockData,
            "expected stockData to be an object"
          );
          assert.equal(
            res.body.stockData.stock,
            "GOOG",
            'expected stock to equal "GOOG"'
          );
          assert.property(
            res.body.stockData,
            "likes",
            'expected to find property "likes"'
          );
          assert.property(
            res.body.stockData,
            "price",
            'expected to find property "price"'
          );
    ...
    test("2 stocks with like", function(done) {
      chai
        ...
        .end((err, res) => {
          ...
          assert.property(res.body.stockData[0], "likes");
          assert.property(res.body.stockData[0], "rel_likes");
          assert.notEqual(res.body.stockData[0], 0);
          assert.equal(res.body.stockData[0].stock, "GOOG");
          assert.property(res.body.stockData[1], "stock");
          assert.property(res.body.stockData[1], "price");
          assert.property(res.body.stockData[1], "likes");
